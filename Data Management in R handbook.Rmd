...
For the first few exercises we will be using a made-up dataset which is presented in 3 different formats on Github (csv, Excel and Access). These are the most commonly used formats for holding data. To download each dataset, click on the link to the dataset and then click on 'View Raw' - this should download a copy of the dataset onto your machine. 
...
Note the data imported via .csv only contains the number of decimal points displayed in Excel so importing directly via Excel is also more accurate.
...
3. Search your C drive for 'odbcad32'and open the version stored in the SysWOW64 folder

4. Click Add > Microsoft Access Driver (*.mdb, *.accdb) > Finish 

5. Enter a short name and description and then Select... > Navigate to where you downloaded the example database

6. Close the ODBC connector


Now we've set up the connection we can access it in R via the RODBC package.

Firstly, we need to create the connection using `odbcConnect` and the short name you have your database. I called mine Example.
...
***To be updated with info from the QESIG Spatial Data Course***
...
For all these examples we will use the iris dataset distributed as part of base R, this is a dataset of measurements of flower characteristics for three species of iris. You could also try the exercises out with the datasets imported in Section 1.

Manipulations of single datasets are generally conducted with the aim of 'tidying up' data for analysis. Hadley Wickham has written a paper on this called 'Tidy Data' linked to the `tidyr` package with some excellent guidelines to point you through this process. There is a link to the paper at the end of this document. We won't use the `tidyr` package much in this course, as most of the functions are wrappers for more basic functions which we will cover.
However, it might be a useful exercise to try and replicate some of the exercises using the `tidyr` package.
